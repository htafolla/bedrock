...
describe('bedrock content integrity', () => {
  ...
  it('ships the full atlas (storm + fruit + war + flesh + readiness + practice + glory)', () => {
    ...
    expect(theLineBody).not.toMatch(/during separation|side of the street|schedule, emotions, and decisions/)
    ...
    expect(yourSideBody).toMatch(/faults on both sides do not cancel the other/)
    expect(yourSideBody).toMatch(/own your part and still refuse to be used or abused/)
    expect(yourSideBody).not.toMatch(/self-eras/)
    ...
    expect(yourSideBody).not.toMatch(/record-keeping|stop the ledger|keeps no record/)
