...
describe('bedrock content integrity', () => {
  ...
  it('ships the full atlas (storm + fruit + war + flesh + readiness + practice + glory)', () => {
    ...
    expect(killText).toMatch(/fear drives the actions of the flesh|reactions or reflexes|before action/)
    ...
    // Core verses as parenthetical citation → verse chips in UI (not a plain blob list)
    expect(killText).toMatch(/\(james 1:19.*2 timothy 1:7.*galatians 5:16/i)
