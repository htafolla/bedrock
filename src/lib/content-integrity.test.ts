...
describe('bedrock content integrity', () => {
  ...
  it('ships the full atlas (storm + fruit + war + flesh + readiness + practice + glory)', () => {
    ...
    expect(killText).toMatch(/you're in a fight|going to explode|master the flesh/)
    expect(killText).toMatch(/fear drives the flesh|reaction and reflex|before action/)
