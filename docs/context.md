# Bedrock — Full Context (SSOT)

**Updated:** 2026-07-24  
**Final product name:** Bedrock  
**Original working title:** The Hitchhiker’s Guild · Love · Living · Enduring

## Overall goal

Bedrock is a **field guide to Love · Living · Enduring**.

It defines the core first principles needed to navigate a troubled life — for people who struggle to hold the Word of God in the heat of the moment.

It is meant to be:

- Full of **truth** (Scripture-rooted first principles)
- Full of **usable brain hacks** (short, steel, pressure-ready reframes)
- Full of **short prayers** (words when you cannot find your own)
- A **navigable web**, not only a linear scroll
- Permanent (IPFS + on-chain), so the testimony is hard to erase

**Why “Bedrock”:** these are first principles that remain true when the ground is shaking.

Internal framing (author): *first principles to “hack the matrix” of flesh, fear, and false narrative — by Spirit, not gimmick.* Public tone stays steel and restrained.

## What it is

A personal, curated set of biblical first principles.

A **working field manual** for enduring suffering, walking by the Spirit, and standing firm when everything is on fire.

### Core opening lines

> I hold these things to be true. A lifetime to master them.  
> Though I fall, I get back up. Out of the fire a crucible emerges.

## Origin & purpose

Forged under real pressure — not theory. Written by a disciple of Jesus Christ: a fallen man who works hard to do good and be good, and still falls. The return path: Spirit over flesh, rise after fall, truth over self-deception.

**Product identity:** a Hitchhiker’s field guide to Love · Living · Enduring — first principles for the road. Private fire may have forged it; the public gift is the map, not the headline.

A response to:

> What remains true when the ground is shaking?

### Sealed word (optional open, not the product pitch)

**First (short):**

> This is a testament to Him that through the fire He was always with me.  
> I have found a crucible in the rubble. A hidden gem only He could fashion.

**Deeper (poem link — “For a deeper understanding”):** backstory poem of fall, unheeded warning, discipline, mercy, and restoring others.

**Public framing:** Bedrock is a Hitchhiker’s field guide to Love · Living · Enduring — first principles for the road. The sealed word is optional; the product is the map.

## The dichotomy (two dual systems)

### 1. Verse dichotomy (access)

| Layer | Source | Purpose |
|-------|--------|---------|
| Primary | Bible Gateway (direct verse) | Human-readable, familiar, searchable |
| Secondary | IPFS / on-chain | Permanent, censorship-resistant |

### 2. Field dichotomy (how you use a chamber)

| Layer | What it is | Job under fire |
|-------|------------|----------------|
| **Truth** | First principles + Scripture | What is solid |
| **Hack** | Short cognitive / obedience reframe | What to do with your mind and will *now* |
| **Prayer** | Short prayer when words fail | What to speak to God *now* |

Together they form a **navigable web of truth**: chambers link to related chambers (fall → confess → Spirit → grace → hope).

## Tone & design philosophy

| Prefer | Over |
|--------|------|
| Steel | Sentiment |
| Restraint | Spectacle |
| Usable under pressure | Merely inspirational |
| Obedience, endurance, Spirit-walking, forgiveness, waiting, hope of glory | Heavy emotional-processing language |
| Lived | Only read |
| First principles | Platform-dependent fluff |

Visual language: quiet cathedral / forge / crucible of light and stone — residual heat, light beams, solid ground.

## Structure (35 major sections)

God First · His Power and Beauty · His Promises · His Provision · The Lord’s Prayer · The Cross and Our Justification · He Is For You · His Grace Is Sufficient · The Meaning of Life · Deny Yourself · Walk by the Spirit · God on Marriage · Marriage Covenant · Love and Patience · Count the Trial as Joy · Wait on the Lord · Lament and Pour Out Your Heart · A Broken and Contrite Heart · Guard Your Heart and Mouth · Be Quick to Listen · Restore Gently and Give Time · Confess and Be Cleansed · Walk in Honesty and Truth · Choose Selfless Love · Do Not Repay Evil with Evil · Leave Vengeance to the Lord · Forgive as You Have Been Forgiven · One Another in the Body · Trust in the Lord · Do Not Fear · Fear · Loss · Renew Your Mind · Take Every Thought Captive · The Full Armor of God · Hope of Glory (crowns / rewards) · The Righteous Fall

Every section ends with supporting Scripture references. Chambers may also carry **hacks**, **prayers**, and **related** links.

## Key theological emphases

- Walking by the Spirit vs. the flesh (Galatians 5)
- Fruit of the Spirit as anti-pattern to rage, impatience, control, negativity, name-calling
- Marriage as lifelong covenant (strong stance against easy divorce)
- Suffering → perseverance → character → hope
- Waiting on the Lord, being still, not fretting
- Forgiveness as non-negotiable
- The righteous fall — and rise again (not pretend sinlessness)
- Dual reality of present pain and future glory / crowns

## Future vision — 3D + permanence

Immersive 3D web experience: quiet cathedral / forge / crucible.

| Requirement | Detail |
|-------------|--------|
| Dual verse links | Primary: public Bible; Secondary: on-chain / IPFS |
| Document permanence | Full content on IPFS + content hash on-chain (**Sui preferred**) |
| Testimony layer | Optional sealed / soulbound mint |
| Progressive enhancement | Full atmosphere on capable devices; elegant 2D mobile fallback |
| Goal | Testimony cannot be easily erased |

## Current status

| Layer | State |
|-------|--------|
| Content | Field-manual quality (35 chambers) |
| Name | Locked: **Bedrock** |
| Dichotomy model | Truth + hacks + prayers + related web (seeded) |
| App | Phase 1 reading experience + light 3D shell |
| Tech spec | v0.1; content SSOT in `src/content/bedrock.json` |
